...
static int originators_callback(struct nl_msg *msg, void *arg) {
  ...
  if (attrs[BATADV_ATTR_THROUGHPUT]) {
    // AIRTIME в микросекундах — просто берём значение как есть
    uint32_t airtime = nla_get_u32(attrs[BATADV_ATTR_THROUGHPUT]);

    if (!(opts->read_opt & USE_BAT_HOSTS)) {
      printf(" %c %02x:%02x:%02x:%02x:%02x:%02x %4i.%03is (%9u) "
             "%02x:%02x:%02x:%02x:%02x:%02x [%10s]\n",
             c, orig[0], orig[1], orig[2], orig[3], orig[4], orig[5],
             last_seen_secs, last_seen_msecs, airtime, neigh[0], neigh[1],
             neigh[2], neigh[3], neigh[4], neigh[5], ifname);
    } else {
      bat_host = bat_hosts_find_by_mac((char *)orig);
      if (bat_host)
        printf(" %c %17s ", c, bat_host->name);
      else
        printf(" %c %02x:%02x:%02x:%02x:%02x:%02x ", c, orig[0], orig[1],
               orig[2], orig[3], orig[4], orig[5]);
      printf("%4i.%03is (%9u) ", last_seen_secs, last_seen_msecs, airtime);
      bat_host = bat_hosts_find_by_mac((char *)neigh);
      if (bat_host)
        printf(" %c %17s ", c, bat_host->name);
      else
        printf(" %02x:%02x:%02x:%02x:%02x:%02x ", neigh[0], neigh[1], neigh[2],
  ...
  if (attrs[BATADV_ATTR_TQ]) {
    ...
    if (!(opts->read_opt & USE_BAT_HOSTS)) {
      printf(" %c %02x:%02x:%02x:%02x:%02x:%02x %4i.%03is   (%3i) "
             "%02x:%02x:%02x:%02x:%02x:%02x [%10s]\n",
             c, orig[0], orig[1], orig[2], orig[3], orig[4], orig[5],
             last_seen_secs, last_seen_msecs, tq, neigh[0], neigh[1], neigh[2],
             neigh[3], neigh[4], neigh[5], ifname);
    } else {
      bat_host = bat_hosts_find_by_mac((char *)orig);
      if (bat_host)
        printf(" %c %17s ", c, bat_host->name);
        ...
        printf(" %c %02x:%02x:%02x:%02x:%02x:%02x ", c, orig[0], orig[1],
               orig[2], orig[3], orig[4], orig[5]);
      printf("%4i.%03is   (%3i) ", last_seen_secs, last_seen_msecs, tq);
      bat_host = bat_hosts_find_by_mac((char *)neigh);
      if (bat_host)
...
static int netlink_print_originators(struct state *state, char *orig_iface,
  ...
  if (!strcmp("BATMAN_IV", algo_name_buf))
    header = "   Originator        last-seen (#/255) Nexthop           "
             "[outgoingIF]\n";
  if (!strcmp("BATMAN_V", algo_name_buf))
    header = "   Originator        last-seen ( airtime)  Nexthop           "
             "[outgoingIF]\n";
