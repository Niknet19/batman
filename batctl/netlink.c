...
static int info_callback(struct nl_msg *msg, void *arg) {
  ...
  if (attrs[BATADV_ATTR_HARD_IFNAME]) {
    ...
    case BATADV_CMD_GET_BLA_CLAIM:
      ret = asprintf(&extra_info, ", group id: 0x%04x", bla_group_id);
      if (ret < 0)
        extra_info = NULL;
      break;
    default:
    ...
    ret = asprintf(
        &opts->remaining_header,
        "[B.A.T.M.A.N. adv %s, MainIF/MAC: %s/%02x:%02x:%02x:%02x:%02x:%02x "
        "(%s/%02x:%02x:%02x:%02x:%02x:%02x %s)%s]\n%s",
        version, primary_if, primary_mac[0], primary_mac[1], primary_mac[2],
        primary_mac[3], primary_mac[4], primary_mac[5], mesh_name, mesh_mac[0],
        mesh_mac[1], mesh_mac[2], mesh_mac[3], mesh_mac[4], mesh_mac[5],
    ...
    ret = asprintf(&opts->remaining_header, "BATMAN mesh %s disabled\n",
                   mesh_name);
    if (ret < 0)
      opts->remaining_header = NULL;
  }
...
char *netlink_get_info(struct state *state, uint8_t nl_cmd,
                       const char *header) {
...
int netlink_print_common(struct state *state, char *orig_iface, int read_opt,
  ...
  do {
    ...
    if (!(read_opt & SKIP_HEADER))
      opts.remaining_header = netlink_get_info(state, nl_cmd, header);
    ...
    nla_put_u32(msg, BATADV_ATTR_MESH_IFINDEX, state->mesh_ifindex);
    if (hardifindex)
      nla_put_u32(msg, BATADV_ATTR_HARD_IFINDEX, hardifindex);
...
int netlink_query_common(struct state *state, unsigned int mesh_ifindex,
                         ...
                         nl_recvmsg_msg_cb_t attribute_cb, int flags,
  ...
  genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, state->batadv_family, 0, flags,
              nl_cmd, 1);
...
static int translate_mac_netlink_cb(struct nl_msg *msg, void *arg) {
  ...
  opts =
      container_of(query_opts, struct translate_mac_netlink_opts, query_opts);
...
int translate_mac_netlink(struct state *state, const struct ether_addr *mac,
  ...
  ret = netlink_query_common(
      state, state->mesh_ifindex, BATADV_CMD_GET_TRANSTABLE_GLOBAL,
      translate_mac_netlink_cb, NULL, NLM_F_DUMP, &opts.query_opts);
  if (ret < 0)
    return ret;
...
static int get_nexthop_netlink_cb(struct nl_msg *msg, void *arg) {
  ...
  opts = container_of(query_opts, struct get_nexthop_netlink_opts, query_opts);
...
int get_nexthop_netlink(struct state *state, const struct ether_addr *mac,
  ...
  ret = netlink_query_common(state, state->mesh_ifindex,
                             BATADV_CMD_GET_ORIGINATORS, get_nexthop_netlink_cb,
                             NULL, NLM_F_DUMP, &opts.query_opts);
  if (ret < 0)
    return ret;
...
static int get_primarymac_netlink_cb(struct nl_msg *msg, void *arg) {
  ...
  opts =
      container_of(query_opts, struct get_primarymac_netlink_opts, query_opts);
...
int get_primarymac_netlink(struct state *state, uint8_t *primarymac) {
  ...
  ret = netlink_query_common(
      state, state->mesh_ifindex, BATADV_CMD_GET_MESH_INFO,
      get_primarymac_netlink_cb, NULL, 0, &opts.query_opts);
  if (ret < 0)
    return ret;
...
static int get_algoname_netlink_cb(struct nl_msg *msg, void *arg) {
  ...
  opts = container_of(query_opts, struct get_algoname_netlink_opts, query_opts);
...
int get_algoname_netlink(struct state *state, unsigned int mesh_ifindex,
  ...
  ret =
      netlink_query_common(state, mesh_ifindex, BATADV_CMD_GET_MESH,
                           get_algoname_netlink_cb, NULL, 0, &opts.query_opts);
  if (ret < 0)
    return ret;
