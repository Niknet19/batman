...
int batadv_originator_init(struct batadv_priv *bat_priv) {
  ...
  queue_delayed_work(batadv_event_workqueue, &bat_priv->orig_work,
...
void batadv_hardif_neigh_release(struct kref *ref) {
  ...
  hardif_neigh = container_of(ref, struct batadv_hardif_neigh_node, refcount);
...
void batadv_neigh_node_release(struct kref *ref) {
  ...
  hlist_for_each_entry_safe(neigh_ifinfo, node_tmp, &neigh_node->ifinfo_list,
                            list) {
...
batadv_orig_ifinfo_get(struct batadv_orig_node *orig_node,
  ...
  rcu_read_lock();
  hlist_for_each_entry_rcu(tmp, &orig_node->ifinfo_list, list) {
...
batadv_orig_ifinfo_new(struct batadv_orig_node *orig_node,
  ...
  hlist_add_head_rcu(&orig_ifinfo->list, &orig_node->ifinfo_list);
out:
...
batadv_neigh_ifinfo_get(struct batadv_neigh_node *neigh,
                        struct batadv_hard_iface *if_outgoing) {
  struct batadv_neigh_ifinfo *neigh_ifinfo = NULL, *tmp_neigh_ifinfo;

  rcu_read_lock();
  hlist_for_each_entry_rcu(tmp_neigh_ifinfo, &neigh->ifinfo_list, list) {
...
batadv_hardif_neigh_get(const struct batadv_hard_iface *hard_iface,
  ...
  rcu_read_lock();
  hlist_for_each_entry_rcu(tmp_hardif_neigh, &hard_iface->neigh_list, list) {
...
batadv_neigh_node_create(struct batadv_orig_node *orig_node,
  ...
  hardif_neigh =
      batadv_hardif_neigh_get_or_create(hard_iface, neigh_addr, orig_node);
  if (!hardif_neigh)
    goto out;
...
void batadv_orig_node_release(struct kref *ref) {
  ...
  hlist_for_each_entry_safe(neigh_node, node_tmp, &orig_node->neigh_list,
                            list) {
    hlist_del_rcu(&neigh_node->list);
    batadv_neigh_node_put(neigh_node);
  }

  hlist_for_each_entry_safe(orig_ifinfo, node_tmp, &orig_node->ifinfo_list,
                            list) {
...
void batadv_originator_free(struct batadv_priv *bat_priv) {
  ...
  for (i = 0; i < hash->size; i++) {
    ...
    hlist_for_each_entry_safe(orig_node, node_tmp, head, hash_entry) {
...
static void batadv_purge_neigh_ifinfo(struct batadv_priv *bat_priv,
  ...
  hlist_for_each_entry_safe(neigh_ifinfo, node_tmp, &neigh->ifinfo_list, list) {
...
static bool batadv_purge_orig_node(struct batadv_priv *bat_priv,
  ...
  if (batadv_has_timed_out(orig_node->last_seen, 2 * BATADV_PURGE_TIMEOUT)) {
    batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
               ...
               orig_node->orig, jiffies_to_msecs(orig_node->last_seen));
    return true;
  }
  ...
  /* first for NULL ... */
  best_neigh_node =
      batadv_find_best_neighbor(bat_priv, orig_node, BATADV_IF_DEFAULT);
  ...
  netdev_for_each_lower_private_rcu(bat_priv->mesh_iface, hard_iface, iter) {
    ...
    best_neigh_node =
        batadv_find_best_neighbor(bat_priv, orig_node, hard_iface);
    batadv_update_route(bat_priv, orig_node, hard_iface, best_neigh_node);
    batadv_neigh_node_put(best_neigh_node);
