...
static void _batadv_update_route(struct batadv_priv *bat_priv,
  ...
  curr_router = rcu_replace_pointer(orig_ifinfo->router, neigh_node, true);
  spin_unlock_bh(&orig_node->neigh_list_lock);
  batadv_orig_ifinfo_put(orig_ifinfo);

  /* route deleted */
  if (curr_router && !neigh_node) {
    batadv_dbg(BATADV_DBG_ROUTES, bat_priv, "Deleting route towards: %pM\n",
               orig_node->orig);
...
int batadv_recv_icmp_packet(struct sk_buff *skb,
  ...
  if ((icmph->msg_type == BATADV_ECHO_REPLY ||
    ...
    ether_addr_copy(icmp_packet_rr->rr[icmp_packet_rr->rr_cur], ethhdr->h_dest);
...
batadv_find_router(struct batadv_priv *bat_priv,
  ...
  hlist_for_each_entry_rcu(cand, &orig_node->ifinfo_list, list) {
     ...
     * considered
     */
    if (!bao->neigh.is_similar_or_better(cand_router, cand->if_outgoing, router,
                                         recv_if))
      goto next;
...
static int batadv_route_unicast_packet(struct sk_buff *skb,
  ...
  if (unicast_packet->ttl < 2) {
    pr_debug("Warning - can't forward unicast packet from %pM to %pM: ttl "
             "exceeded\n",
  ...
  if (res == NET_XMIT_SUCCESS) {
    ...
    batadv_add_counter(bat_priv, BATADV_CNT_FORWARD_BYTES, len + ETH_HLEN);
  }
...
batadv_reroute_unicast_packet(struct batadv_priv *bat_priv, struct sk_buff *skb,
  ...
  if (batadv_is_my_client(bat_priv, dst_addr, vid)) {
    ...
    orig_node = batadv_transtable_search(bat_priv, NULL, dst_addr, vid);
    if (!orig_node)
      goto out;
...
static bool batadv_check_unicast_ttvn(struct batadv_priv *bat_priv,
  ...
  if (batadv_reroute_unicast_packet(bat_priv, skb, unicast_packet,
    ...
    batadv_dbg_ratelimited(BATADV_DBG_TT, bat_priv,
                           "Rerouting unicast packet to %pM (dst=%pM): TTVN "
                           "mismatch old_ttvn=%u new_ttvn=%u\n",
                           unicast_packet->dest, ethhdr->h_dest, old_ttvn,
                           curr_ttvn);
    return true;
  }
...
int batadv_recv_unicast_packet(struct sk_buff *skb,
  ...
  if (batadv_is_my_mac(bat_priv, unicast_packet->dest)) {
    ...
    if (orig_node_gw) {
      is_gw = batadv_bla_is_backbone_gw(skb, orig_node_gw, hdr_size);
      batadv_orig_node_put(orig_node_gw);
      if (is_gw) {
        batadv_dbg(BATADV_DBG_BLA, bat_priv,
                   "%s(): Dropped unicast pkt received from another backbone "
                   "gw %pM.\n",
...
int batadv_recv_unicast_tvlv(struct sk_buff *skb,
  ...
  ret = batadv_tvlv_containers_process(bat_priv, BATADV_UNICAST_TVLV, NULL, skb,
...
int batadv_recv_frag_packet(struct sk_buff *skb,
  ...
  if (batadv_check_unicast_packet(bat_priv, skb, sizeof(*frag_packet)) < 0)
    goto free_skb;
   ...
   */
  if (skb) {
    batadv_batman_skb_recv(skb, recv_if->net_dev, &recv_if->batman_adv_ptype,
                           NULL);
    /* skb was consumed */
    skb = NULL;
  }
...
int batadv_recv_bcast_packet(struct sk_buff *skb,
  ...
  if (batadv_window_protected(bat_priv, seq_diff, BATADV_BCAST_MAX_AGE,
