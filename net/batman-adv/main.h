...
/* B.A.T.M.A.N. V — Airtime Metric (802.11s-style) */
#define BATADV_AIRTIME_MAX_VALUE                                               \
  0xFFFFFFFF /* максимальная стоимость (худший линк) */
#define BATADV_AIRTIME_OVERHEAD                                                \
  300 /* O в микросекундах: DIFS + backoff + preamble + SIFS + ACK */
#define BATADV_AIRTIME_TEST_FRAME_SIZE                                         \
  8192                               /* B_t в битах (1024 байта по 802.11s) */
#define BATADV_AIRTIME_EWMA_WEIGHT 4 /* вес для EWMA-сглаживания e_pt (1/4) */
#define BATADV_AIRTIME_HALF_DUPLEX_PENALTY                                     \
  2 /* множитель airtime для полудуплекса */

/* *B.A.T.M.A.N.V — Weighted Airtime Metric(WAM) */

/* Node weight defaults (no penalty by default) */
#define BATADV_NODE_PRIORITY_DEFAULT 128 /* 1-255, 128 = neutral (×1.0) */
#define BATADV_DUTY_CYCLE_DEFAULT 100    /* 0-100, 100 = always active (×1.0) */
#define BATADV_POWER_CLASS_DEFAULT                                             \
  1 /* 1=mains, 2=battery, 3=energy_harvesting */

/* Node weight scaling factor (тысячные доли) */
#define BATADV_WEIGHT_SCALE 1000

/* TVLV type for node weight announcement */
#define BATADV_TVLV_NODE_WEIGHT 0x10

/* интервал сброса счётчиков ELP для подсчёта потерь (мс) */
#define BATADV_ELP_LOSS_INTERVAL 1000
...
int batadv_recv_handler_register(
    u8 packet_type,
    int (*recv_handler)(struct sk_buff *, struct batadv_hard_iface *));
