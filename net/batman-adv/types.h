...
struct batadv_orig_node {
  ...
  /**
   * @node_priority: приоритет узла-источника (из TVLV)
   */
  u8 node_priority;

  /**
   * @duty_cycle: duty cycle узла-источника (из TVLV)
   */
  u8 duty_cycle;

  /**
   * @power_class: класс питания узла-источника (из TVLV)
   */
  u8 power_class;
};
...
struct batadv_hardif_neigh_node_bat_v {
  ...
  u32 airtime_cost;   // итоговая airtime-стоимость до соседа (мкс)
  u32 base_airtime;   // базовая стоимость без учёта потерь
  u32 elp_received;   // счётчик полученных ELP-пакетов за интервал
  u32 elp_expected;   // счётчик ожидаемых ELP-пакетов за интервал
  u32 elp_last_seqno; // последний порядковый номер ELP
  u32 e_pt_smoothed;  // сглаженная частота ошибок кадра (в тысячных долях,
                      // 0-1000)
...
struct batadv_neigh_ifinfo_bat_v {
   ...
   *  neigh
   */
  u32 airtime_cost; // airtime-стоимость пути через этого соседа на этом
                    // интерфейсе (мкс)
...
struct batadv_priv {
  ...
  /**
   * @node_priority: административный приоритет узла (1-255, 128 = нейтральный)
   */
  atomic_t node_priority;

  /**
   * @duty_cycle: процент времени в активном режиме (0-100, 100 = всегда)
   */
  atomic_t duty_cycle;

  /**
   * @power_class: класс питания (1=mains, 2=battery, 3=energy_harvesting)
   */
  atomic_t power_class;
...
struct batadv_algo_neigh_ops {
  ...
  void (*dump)(struct sk_buff *msg, struct netlink_callback *cb,
               struct batadv_priv *priv, struct batadv_hard_iface *hard_iface);
};
...
struct batadv_tvlv_handler {
  ...
  void (*ogm_handler)(struct batadv_priv *bat_priv,
                      struct batadv_orig_node *orig, u8 flags, void *tvlv_value,
                      u16 tvlv_value_len);
   ...
   */
  int (*unicast_handler)(struct batadv_priv *bat_priv, u8 *src, u8 *dst,
...
/**
 * struct batadv_tvlv_node_weight - TVLV container for node weight parameters
 */
struct batadv_tvlv_node_weight {
  /** @priority: административный приоритет (1-255) */
  u8 priority;

  /** @duty_cycle: процент активности (0-100) */
  u8 duty_cycle;

  /** @power_class: класс питания (1-3) */
  u8 power_class;
};
