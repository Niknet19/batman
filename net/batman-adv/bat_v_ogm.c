...
struct batadv_orig_node *batadv_v_ogm_orig_get(struct batadv_priv *bat_priv,
  ...
  hash_added =
      batadv_hash_add(bat_priv->orig_hash, batadv_compare_orig,
                      batadv_choose_orig, orig_node, &orig_node->hash_entry);
  if (hash_added != 0) {
 ...
 */
static void
batadv_v_ogm_start_queue_timer(struct batadv_hard_iface *hard_iface) {
...
static void batadv_v_ogm_start_timer(struct batadv_priv *bat_priv) {
   ...
   */
  pr_err("=== AIRTIME DEBUG: batadv_v_ogm_start_timer called ===\n");
  if (delayed_work_pending(&bat_priv->bat_v.ogm_wq)) {
    pr_err("=== AIRTIME DEBUG: OGM work already pending, skipping ===\n");
    return;
  }

  msecs = atomic_read(&bat_priv->orig_interval) - BATADV_JITTER;
  msecs += get_random_u32_below(2 * BATADV_JITTER);
  pr_err("=== AIRTIME DEBUG: OGM timer armed, %lu ms ===\n", msecs);
...
static void batadv_v_ogm_send_meshif(struct batadv_priv *bat_priv) {
  ...
  netdev_for_each_lower_private_rcu(bat_priv->mesh_iface, hard_iface, iter) {
    ...
    pr_err("Sending own OGM2 packet (originator %pM, seqno %u, airtime "
           "%u, TTL %d) on interface %s [%pM]\n",
           ogm_packet->orig, ntohl(ogm_packet->seqno),
           ntohl(ogm_packet->throughput), ogm_packet->ttl,
           hard_iface->net_dev->name, hard_iface->net_dev->dev_addr);

    batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
               "Sending own OGM2 packet (originator %pM, seqno %u, airtime "
               "%u, TTL %d) on interface %s [%pM]\n",
               ogm_packet->orig, ntohl(ogm_packet->seqno),
               ntohl(ogm_packet->throughput), ogm_packet->ttl,
               hard_iface->net_dev->name, hard_iface->net_dev->dev_addr);
...
static void batadv_v_ogm_send(struct work_struct *work) {
  ...
  pr_err("=== AIRTIME DEBUG: batadv_v_ogm_send START ===\n");
  ...
  pr_err("=== AIRTIME DEBUG: batadv_v_ogm_send END ===\n");
}
...
int batadv_v_ogm_iface_enable(struct batadv_hard_iface *hard_iface) {
  ...
  pr_err("=== AIRTIME DEBUG: batadv_v_ogm_iface_enable for %s ===\n",
...
/**
 * batadv_v_forward_penalty() - применить штраф к airtime-метрике
 * @bat_priv: the bat priv
 * @if_incoming: интерфейс приёма OGM
 * @if_outgoing: интерфейс передачи OGM
 * @airtime: текущая airtime-стоимость пути
 *
 * Return: airtime-стоимость со штрафом
 */
static u32 batadv_v_forward_penalty(struct batadv_priv *bat_priv,
  ...
  /* Для default-интерфейса штраф не применяем */
  if (if_outgoing == BATADV_IF_DEFAULT)
    return airtime;

  /* Полудуплекс: если входящий и исходящий — один и тот же WiFi,
   * удваиваем стоимость (приём + передача занимают эфир дважды) */
  if (if_incoming == if_outgoing &&
      !(if_incoming->bat_v.flags & BATADV_FULL_DUPLEX))
    return airtime * BATADV_AIRTIME_HALF_DUPLEX_PENALTY;

  /* Общий штраф за хоп (hop penalty) добавляет фиксированный процент */
  return airtime + (airtime * hop_penalty / BATADV_TQ_MAX_VALUE);
}
...
static bool batadv_v_ogm_route_update(struct batadv_priv *bat_priv,
  ...
  u32 router_airtime, neigh_airtime;
  ...
  if (router) {
    ...
    router_airtime = router_ifinfo->bat_v.airtime_cost;
    neigh_airtime = neigh_ifinfo->bat_v.airtime_cost;

    /* если новый сосед имеет больший (худший) airtime и seqno не сильно новее —
     * не переключаемся
     */
    if (neigh_seq_diff < BATADV_OGM_MAX_ORIGDIFF &&
        router_airtime <= neigh_airtime)
      goto out;
  }
  /* --- КОНЕЦ ИНВЕРСИИ --- */
 ...
 * @if_outgoing: the interface for which the packet should be considered
 */
static void batadv_v_ogm_process_per_outif(
    struct batadv_priv *bat_priv, const struct ethhdr *ethhdr,
    const struct batadv_ogm2_packet *ogm2, struct batadv_orig_node *orig_node,
    struct batadv_neigh_node *neigh_node, struct batadv_hard_iface *if_incoming,
    struct batadv_hard_iface *if_outgoing) {
  int seqno_age;
  bool forward;
  ...
  if (forward)
    batadv_v_ogm_forward(bat_priv, ogm2, orig_node, neigh_node, if_incoming,
                         if_outgoing);
}
...
static void batadv_v_ogm_process(const struct sk_buff *skb, int ogm_offset,
  ...
  struct batadv_hard_iface *hard_iface;
  struct batadv_ogm2_packet *ogm_packet;
  /* AIRTIME: ogm_airtime — стоимость пути из OGM (мкс),
   *          link_airtime — стоимость линка до соседа (мкс) */
  u32 ogm_airtime, link_airtime, path_airtime;
  struct list_head *iter;
  int ret;
  ...
  /* AIRTIME: поле ogm_packet->throughput теперь несёт airtime-стоимость пути
   */
  ogm_airtime = ntohl(ogm_packet->throughput);

  batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
             "Received OGM2 packet via NB: %pM, IF: %s [%pM] (from OG: %pM, "
             "seqno %u, airtime %u us, TTL %u, V %u, tvlv_len %u)\n",
             ethhdr->h_source, if_incoming->net_dev->name,
             if_incoming->net_dev->dev_addr, ogm_packet->orig,
             ntohl(ogm_packet->seqno), ogm_airtime, ogm_packet->ttl,
             ogm_packet->version, ntohs(ogm_packet->tvlv_len));

  /* AIRTIME: отбрасываем свой собственный OGM */
  ...
  /* AIRTIME: если стоимость равна MAX_VALUE — маршрут непригоден, дропаем */
  if (ogm_airtime == BATADV_AIRTIME_MAX_VALUE) {
    batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
               "Drop packet: originator packet with max airtime (unusable)\n");
    return;
  }
  ...
  /*
   * AIRTIME: обновление метрики пути через сложение.
   *
   * В отличие от throughput-метрики, где path = min(ogm, link),
   * airtime-метрика аддитивна: каждый хоп добавляет свою стоимость.
   *
  ...
  netdev_for_each_lower_private_rcu(bat_priv->mesh_iface, hard_iface, iter) {
    ...
    ret = batadv_hardif_no_broadcast(hard_iface, ogm_packet->orig,
                                     hardif_neigh->orig);
    ...
    if (ret) {
      ...
      batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
                 ...
                 ogm_packet->orig, hard_iface->net_dev->name, type);
 ...
 *
 * Return: NET_RX_SUCCESS and consume the skb on success or returns
 * NET_RX_DROP (without freeing the skb) on failure
 */
int batadv_v_ogm_packet_recv(struct sk_buff *skb,
  ...
  while (batadv_v_ogm_aggr_packet(ogm_offset, skb_headlen(skb), ogm_packet)) {
...
int batadv_v_ogm_init(struct batadv_priv *bat_priv) {
  ...
  pr_err("=== AIRTIME DEBUG: batadv_v_ogm_init called ===\n");
