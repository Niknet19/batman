...
static const struct nla_policy batadv_netlink_policy[NUM_BATADV_ATTR] = {
    ...
    [BATADV_ATTR_NODE_PRIORITY] = {.type = NLA_U8},
    [BATADV_ATTR_DUTY_CYCLE] = {.type = NLA_U8},
    [BATADV_ATTR_POWER_CLASS] = {.type = NLA_U8},
};
 ...
 */
static int batadv_netlink_get_ifindex(const struct nlmsghdr *nlh,
                                      int attrtype) {
...
static int batadv_netlink_mesh_fill_ap_isolation(struct sk_buff *msg,
  ...
  return nla_put_u8(msg, BATADV_ATTR_AP_ISOLATION_ENABLED, !!ap_isolation);
}
...
static int batadv_netlink_mesh_fill(struct sk_buff *msg,
                                    struct batadv_priv *bat_priv,
                                    enum batadv_nl_commands cmd, u32 portid,
                                    u32 seq, int flags) {
  ...
  if (nla_put_string(msg, BATADV_ATTR_VERSION, BATADV_SOURCE_VERSION) ||
      nla_put_string(msg, BATADV_ATTR_ALGO_NAME, bat_priv->algo_ops->name) ||
      nla_put_u32(msg, BATADV_ATTR_MESH_IFINDEX, mesh_iface->ifindex) ||
      nla_put_string(msg, BATADV_ATTR_MESH_IFNAME, mesh_iface->name) ||
      nla_put(msg, BATADV_ATTR_MESH_ADDRESS, ETH_ALEN, mesh_iface->dev_addr) ||
      nla_put_u8(msg, BATADV_ATTR_TT_TTVN, (u8)atomic_read(&bat_priv->tt.vn)))
  ...
  if (primary_if && primary_if->if_status == BATADV_IF_ACTIVE) {
    ...
    if (nla_put_u32(msg, BATADV_ATTR_HARD_IFINDEX, hard_iface->ifindex) ||
        nla_put_string(msg, BATADV_ATTR_HARD_IFNAME, hard_iface->name) ||
        nla_put(msg, BATADV_ATTR_HARD_ADDRESS, ETH_ALEN, hard_iface->dev_addr))
      goto nla_put_failure;
  }
  ...
  if (nla_put_u32(msg, BATADV_ATTR_ISOLATION_MARK, bat_priv->isolation_mark))
    goto nla_put_failure;
  ...
  if (nla_put_u8(msg, BATADV_ATTR_GW_MODE, atomic_read(&bat_priv->gw.mode)))
    goto nla_put_failure;
  ...
  if (nla_put_u8(msg, BATADV_ATTR_NODE_PRIORITY,
                 atomic_read(&bat_priv->node_priority)))
    goto nla_put_failure;

  if (nla_put_u8(msg, BATADV_ATTR_DUTY_CYCLE,
                 atomic_read(&bat_priv->duty_cycle)))
    goto nla_put_failure;

  if (nla_put_u8(msg, BATADV_ATTR_POWER_CLASS,
                 atomic_read(&bat_priv->power_class)))
    goto nla_put_failure;
...
static int batadv_netlink_notify_mesh(struct batadv_priv *bat_priv) {
  ...
  ret = batadv_netlink_mesh_fill(msg, bat_priv, BATADV_CMD_SET_MESH, 0, 0, 0);
  if (ret < 0) {
    nlmsg_free(msg);
    return ret;
  }

  genlmsg_multicast_netns(&batadv_netlink_family, dev_net(bat_priv->mesh_iface),
                          msg, 0, BATADV_NL_MCGRP_CONFIG, GFP_KERNEL);
...
static int batadv_netlink_set_mesh(struct sk_buff *skb,
  ...
  if (info->attrs[BATADV_ATTR_BRIDGE_LOOP_AVOIDANCE_ENABLED]) {
    ...
    atomic_set(&bat_priv->bridge_loop_avoidance, !!nla_get_u8(attr));
  ...
  if (info->attrs[BATADV_ATTR_DISTRIBUTED_ARP_TABLE_ENABLED]) {
    ...
    atomic_set(&bat_priv->distributed_arp_table, !!nla_get_u8(attr));
  ...
  if (info->attrs[BATADV_ATTR_NODE_PRIORITY]) {
    u8 val = nla_get_u8(info->attrs[BATADV_ATTR_NODE_PRIORITY]);
    atomic_set(&bat_priv->node_priority, val);
  }

  if (info->attrs[BATADV_ATTR_DUTY_CYCLE]) {
    u8 val = nla_get_u8(info->attrs[BATADV_ATTR_DUTY_CYCLE]);
    if (val <= 100)
      atomic_set(&bat_priv->duty_cycle, val);
  }

  if (info->attrs[BATADV_ATTR_POWER_CLASS]) {
    u8 val = nla_get_u8(info->attrs[BATADV_ATTR_POWER_CLASS]);
    if (val >= 1 && val <= 3)
      atomic_set(&bat_priv->power_class, val);
  }
...
int batadv_netlink_tpmeter_notify(struct batadv_priv *bat_priv, const u8 *dst,
  ...
  hdr = genlmsg_put(msg, 0, 0, &batadv_netlink_family, 0, BATADV_CMD_TP_METER);
  if (!hdr) {
    ret = -ENOBUFS;
    goto err_genlmsg;
  }
  ...
  genlmsg_multicast_netns(&batadv_netlink_family, dev_net(bat_priv->mesh_iface),
                          msg, 0, BATADV_NL_MCGRP_TPMETER, GFP_KERNEL);
...
static int batadv_netlink_tp_meter_start(struct sk_buff *skb,
  ...
  msg_head = genlmsg_put(msg, info->snd_portid, info->snd_seq,
                         &batadv_netlink_family, 0, BATADV_CMD_TP_METER);
  if (!msg_head) {
    ret = -ENOBUFS;
    goto out;
  }
...
static int batadv_netlink_hardif_fill(struct sk_buff *msg,
  ...
  if (nla_put_u32(msg, BATADV_ATTR_MESH_IFINDEX, bat_priv->mesh_iface->ifindex))
    goto nla_put_failure;

  if (nla_put_string(msg, BATADV_ATTR_MESH_IFNAME, bat_priv->mesh_iface->name))
    goto nla_put_failure;

  if (nla_put_u32(msg, BATADV_ATTR_HARD_IFINDEX, net_dev->ifindex) ||
      nla_put_string(msg, BATADV_ATTR_HARD_IFNAME, net_dev->name) ||
      nla_put(msg, BATADV_ATTR_HARD_ADDRESS, ETH_ALEN, net_dev->dev_addr))
    goto nla_put_failure;
...
static int batadv_netlink_cmd_get_hardif(struct sk_buff *skb,
  ...
  ret = batadv_netlink_hardif_fill(msg, bat_priv, hard_iface,
                                   BATADV_CMD_GET_HARDIF, info->snd_portid,
                                   info->snd_seq, 0, NULL);
  if (ret < 0) {
    nlmsg_free(msg);
    return ret;
...
static int batadv_netlink_set_hardif(struct sk_buff *skb,
  ...
  if (info->attrs[BATADV_ATTR_THROUGHPUT_OVERRIDE]) {
    ...
    atomic_set(&hard_iface->bat_v.throughput_override, nla_get_u32(attr));
  }
#endif /* CONFIG_BATMAN_ADV_BATMAN_V */
...
static int batadv_netlink_dump_hardif(struct sk_buff *msg,
  ...
  netdev_for_each_lower_private(mesh_iface, hard_iface, iter) {
    ...
    if (batadv_netlink_hardif_fill(msg, bat_priv, hard_iface,
                                   BATADV_CMD_GET_HARDIF, portid,
                                   cb->nlh->nlmsg_seq, NLM_F_MULTI, cb)) {
      i--;
      break;
    }
  }
...
static int batadv_netlink_notify_vlan(struct batadv_priv *bat_priv,
  ...
  ret = batadv_netlink_vlan_fill(msg, bat_priv, vlan, BATADV_CMD_SET_VLAN, 0, 0,
                                 0);
  if (ret < 0) {
    nlmsg_free(msg);
    return ret;
...
struct net_device *batadv_netlink_get_meshif(struct netlink_callback *cb) {
  int ifindex = batadv_netlink_get_ifindex(cb->nlh, BATADV_ATTR_MESH_IFINDEX);
  if (!ifindex)
    return ERR_PTR(-ENONET);

  return batadv_netlink_get_meshif_from_ifindex(sock_net(cb->skb->sk), ifindex);
}
...
batadv_netlink_get_hardif(struct batadv_priv *bat_priv,
                          struct netlink_callback *cb) {
  int ifindex = batadv_netlink_get_ifindex(cb->nlh, BATADV_ATTR_HARD_IFINDEX);
  if (!ifindex)
    return ERR_PTR(-ENONET);

  return batadv_netlink_get_hardif_from_ifindex(bat_priv, sock_net(cb->skb->sk),
                                                ifindex);
}
...
static int batadv_pre_doit(const struct genl_split_ops *ops,
  ...
  if (ops->internal_flags & BATADV_FLAG_NEED_HARDIF) {
    hard_iface = batadv_netlink_get_hardif_from_info(bat_priv, net, info);
...
static void batadv_post_doit(const struct genl_split_ops *ops,
  ...
  if (ops->internal_flags & BATADV_FLAG_NEED_HARDIF && info->user_ptr[1]) {
...
static const struct genl_small_ops batadv_netlink_ops[] = {
    ...
    },
    {
        ...
        .internal_flags = BATADV_FLAG_NEED_MESH | BATADV_FLAG_NEED_HARDIF,
    },
    {
    ...
    },
    {
        ...
        .internal_flags = BATADV_FLAG_NEED_MESH | BATADV_FLAG_NEED_VLAN,
    },
    {
